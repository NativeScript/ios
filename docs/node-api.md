...
The runtime implements [Node-API](https://nodejs.org/api/n-api.html), the ABI-stable C interface Node.js addons are written against. The implementation is Node v26.7.0's own `js_native_api` sources, vendored unmodified under `NativeScript/napi/vendor/`; only the pieces Node implements on top of libuv and its module loader are reimplemented here, against a `CFRunLoop` and the runtime's own `require()`.

An addon written for this runtime is ordinary C or Objective-C++ compiled into the app binary. It is not a `.node` file: nothing here loads shared libraries at runtime, so the addon registers itself from a static constructor and JS reaches it through `require("<name>")`.

```objc
#include <NativeScript/NapiRuntime.h>
...
**Do not use the `NAPI_MODULE` / `NAPI_MODULE_INIT` macros.** They are present in the vendored `node_api.h` and they compile, but all they do is emit the two exported symbols (`napi_register_module_v1`, `node_api_module_get_api_version_v1`) that a `dlopen`-based loader would look for. There is no such loader here, the symbols carry no module name, and only one of each can exist per binary. An addon registered that way is simply never found.
...
```objc
#import <NativeScript/NapiRuntime.h>

napi_env env = NativeScriptNapiEnv();  // or [NapiRuntime env]
if (env != NULL) {
  // ...
}
```
...
**Every Node-API call must happen on the thread that owns the env.** That thread holds the isolate's lock and drives the runloop; entering the isolate from anywhere else deadlocks against class initialization and the runtime's other cross-isolate locking. The API does not check this for you (`napi_make_callback` and blocking threadsafe-function calls are the only two that do), so a wrong-thread call is undefined behaviour rather than an error status.

Two supported ways to get work off that thread:

- **`napi_create_async_work` / `napi_queue_async_work`** for background compute. The `execute` callback runs on a GCD global concurrent queue and must not touch the isolate or the `napi_env` at all; the `complete` callback is posted back to the env's runloop and may. Note the divergence on `napi_delete_async_work` below.
...
Everything in `js_native_api.h` behaves exactly as upstream: it is upstream, compiled unmodified. The differences are confined to the `node_api.h` surface, where Node's implementation depends on libuv, `node::Buffer` or the module loader.

| API | Behaviour here | Why |
| --- | --- | --- |
| `napi_get_uv_event_loop` | Always `napi_generic_failure` | There is no `uv_loop_t`. The runtime drives a `CFRunLoop`; handing out a fabricated loop would be worse than reporting failure. |
| `node_api_get_module_file_name` | Always `napi_generic_failure` | Addons are linked into the app binary, not loaded from a file, and nothing identifies the calling module. |
...
| `napi_ref_threadsafe_function`, `napi_unref_threadsafe_function` | Track the flag, affect nothing; always `napi_ok` | The runloop belongs to the app or the worker and does not exit because an addon released its last reference. |
| `napi_call_threadsafe_function`, blocking mode, called on the env's own thread with a full queue | `napi_would_deadlock` instead of blocking | The thread that would have to drain the queue is the caller. Node blocks regardless and never returns this status; wedging the runloop is worse than a status an addon may not expect. Non-blocking calls still return `napi_queue_full`, and a closed function still returns `napi_closing`. |
...
| `napi_fatal_exception` | Reports through the runtime's error handlers (`error` event, crash reporter) and returns; does not abort | `napi_fatal_error` still aborts the process, as upstream. |
| `napi_is_promise` on a promise made via the global `Promise` | `false` | The runtime replaces the global `Promise` with a Proxy for runloop integration (`promise-proxy.js`), and constructed promises come back Proxy-wrapped — not a V8 `Promise` at the engine level. Promises from `napi_create_promise` and from `async` functions report `true`, and awaiting or `.then`-ing any of them from JS works regardless. |

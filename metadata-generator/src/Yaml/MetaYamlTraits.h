...
struct ScalarTraits<Meta::Version> {
  static void output(const Meta::Version& value, void* context,
                     raw_ostream& out) {
    if (value.Major >= 0) {
      out << value.Major;
      if (value.Minor >= 0) {
  ...
  static StringRef input(StringRef stringValue, void* context,
                         Meta::Version& value) {
    ...
    value.Major = (firstDotIndex != std::string::npos)
                      ? std::stoi(version.substr(0, firstDotIndex))
                      : std::stoi(version);
    if (firstDotIndex != std::string::npos) {
      unsigned long secondDotIndex = version.find(".", firstDotIndex + 1);
      value.Minor = std::stoi(version.substr(
          firstDotIndex + 1, (secondDotIndex != std::string::npos)
                                 ? secondDotIndex - firstDotIndex - 1
                                 : std::string::npos));
      if (secondDotIndex != std::string::npos) {
        value.SubMinor =
            std::stoi(version.substr(secondDotIndex + 1, std::string::npos));
      }
    }

    // TODO: We can validate the version and return non-empty string if the yaml
    // format of the version is invalid
    return StringRef();
  }
...
struct ScalarBitSetTraits<Meta::MetaFlags> {
  static void bitset(IO& io, Meta::MetaFlags& value) {
    io.bitSetCase(value, "IsIosAppExtensionAvailable",
                  Meta::MetaFlags::IsIosAppExtensionAvailable);
    io.bitSetCase(value, "MemberIsOptional", Meta::MetaFlags::MemberIsOptional);
    // io.bitSetCase(value, "HasName",  Meta::MetaFlags::HasName);

    io.bitSetCase(value, "FunctionIsVariadic",
                  Meta::MetaFlags::FunctionIsVariadic);
    io.bitSetCase(value, "FunctionOwnsReturnedCocoaObject",
                  Meta::MetaFlags::FunctionOwnsReturnedCocoaObject);
    io.bitSetCase(value, "FunctionReturnsUnmanaged",
                  Meta::MetaFlags::FunctionReturnsUnmanaged);

    io.bitSetCase(value, "MethodIsVariadic", Meta::MetaFlags::MethodIsVariadic);
    io.bitSetCase(value, "MethodIsNullTerminatedVariadic",
                  Meta::MetaFlags::MethodIsNullTerminatedVariadic);
    io.bitSetCase(value, "MethodOwnsReturnedCocoaObject",
                  Meta::MetaFlags::MethodOwnsReturnedCocoaObject);
    io.bitSetCase(value, "MethodHasErrorOutParameter",
                  Meta::MetaFlags::MethodHasErrorOutParameter);
    io.bitSetCase(value, "MethodIsInitializer",
                  Meta::MetaFlags::MethodIsInitializer);
  }
};
...
struct ScalarEnumerationTraits<Meta::TypeType> {
  static void enumeration(IO& io, Meta::TypeType& value) {
    ...
    io.enumCase(value, "BridgedInterface",
                Meta::TypeType::TypeBridgedInterface);
    ...
    io.enumCase(value, "ExtVector", Meta::TypeType::TypeExtVector);
    io.enumCase(value, "Nullable", Meta::TypeType::TypeNullable);
    io.enumCase(value, "NonNullable", Meta::TypeType::TypeNonNullable);
  }
};
...
struct MappingTraits<std::pair<clang::Module*, std::vector<Meta::Meta*> > > {
  static void mapping(
      IO& io, std::pair<clang::Module*, std::vector<Meta::Meta*> >& module) {
  ...
  static void mapping(IO& io, Meta::Type*& type) {
    ...
    switch (typeType) {
      ...
      case Meta::TypeType::TypeConstantArray: {
        Meta::ConstantArrayType& concreteType =
            type->as<Meta::ConstantArrayType>();
      ...
      case Meta::TypeType::TypeIncompleteArray: {
        Meta::IncompleteArrayType& concreteType =
            type->as<Meta::IncompleteArrayType>();
        io.mapRequired("ArrayType", concreteType.innerType);
        break;
      }
      ...
      case Meta::TypeType::TypeBridgedInterface: {
        Meta::BridgedInterfaceType& concreteType =
            type->as<Meta::BridgedInterfaceType>();
        io.mapRequired("Name", concreteType.name);
        std::string bridgedTo =
            concreteType.isId() ? "id"
                                : (concreteType.bridgedInterface == nullptr
                                       ? "[None]"
                                       : concreteType.bridgedInterface->jsName);
      ...
      case Meta::TypeType::TypeFunctionPointer: {
        Meta::FunctionPointerType& concreteType =
            type->as<Meta::FunctionPointerType>();
      ...
      case Meta::TypeType::TypeStruct: {
        ...
        std::string fullModuleName =
            concreteType.structMeta->module->getFullModuleName();
      ...
      case Meta::TypeType::TypeUnion: {
        ...
        std::string fullModuleName =
            concreteType.unionMeta->module->getFullModuleName();
        io.mapRequired("Module", fullModuleName);
        io.mapRequired("Name", concreteType.unionMeta->name);
        break;
      }
      case Meta::TypeType::TypeAnonymousStruct: {
        Meta::AnonymousStructType& concreteType =
            type->as<Meta::AnonymousStructType>();
        io.mapRequired("Fields", concreteType.fields);
        break;
      }
      case Meta::TypeType::TypeAnonymousUnion: {
        Meta::AnonymousUnionType& concreteType =
            type->as<Meta::AnonymousUnionType>();
        io.mapRequired("Fields", concreteType.fields);
        break;
      }
      ...
      case Meta::TypeType::TypeTypeArgument: {
        Meta::TypeArgumentType& concreteType =
            type->as<Meta::TypeArgumentType>();
        io.mapRequired("Name", concreteType.name);
      ...
      case Meta::TypeType::TypeNonNullable: {
        Meta::NonNullableType& concreteType = type->as<Meta::NonNullableType>();
        io.mapRequired("InnerType", concreteType.innerType);
        break;
      }
      default: {
...
struct MappingTraits<Meta::PropertyMeta*> {
  static void mapping(IO& io, Meta::PropertyMeta*& meta) {
    ...
    if (meta->getter) io.mapRequired("Getter", meta->getter);
    if (meta->setter) io.mapRequired("Setter", meta->setter);
  }
};
...
// Meta *
// These traits check which is the actual run-time type of the meta and forward
// to the corresponding traits.
template <>
  ...
  static void mapping(IO& io, Meta::Meta*& meta) {
    switch (meta->type) {
      ...
      case Meta::MetaType::EnumConstant: {
        Meta::EnumConstantMeta* enumConstantMeta =
            &meta->as<Meta::EnumConstantMeta>();

...
namespace Meta {
bool areRecordFieldListsEqual(const std::vector<RecordField>& vector1,
                              const std::vector<RecordField>& vector2) {
  if (vector1.size() != vector2.size()) {
    return false;
  }

  for (std::vector<RecordField>::size_type i = 0; i < vector1.size(); i++) {
    if ((vector1[i].name != vector2[i].name) ||
        !Utils::areTypesEqual(*vector1[i].encoding, *vector2[i].encoding)) {
      return false;
    }
  }
  return true;
...
bool Utils::areTypesEqual(const Type& type1In, const Type& type2In) {
  // Nullability annotations are wrapper nodes in the type tree and are not
  // significant for equality (e.g. a protocol redeclaration differing only in
  // _Nullable/_Nonnull is the same member), so compare the underlying types.
  const Type& type1 = *type1In.stripNullability();
  const Type& type2 = *type2In.stripNullability();
  if (type1.getType() != type2.getType()) return false;
  ...
  switch (type1.getType()) {
    ...
    case TypeType::TypeConstantArray: {
      ...
      return arrayType1.size == arrayType2.size &&
             areTypesEqual(*arrayType1.innerType, *arrayType2.innerType);
    };
    ...
    case TypeType::TypeFunctionPointer: {
      const FunctionPointerType& functionType1 =
          type1.as<FunctionPointerType>();
      const FunctionPointerType& functionType2 =
          type2.as<FunctionPointerType>();
      return Utils::areTypesEqual(functionType1.signature,
                                  functionType2.signature);
    };
    case TypeType::TypeInterface: {
      ...
      return interfaceType1.interface == interfaceType2.interface &&
             interfaceType1.protocols == interfaceType2.protocols;
    };
    case TypeType::TypeBridgedInterface: {
      const BridgedInterfaceType& interfaceType1 =
          type1.as<BridgedInterfaceType>();
      const BridgedInterfaceType& interfaceType2 =
          type2.as<BridgedInterfaceType>();
...
bool Utils::areTypesEqual(const std::vector<Type*>& vector1,
                          const std::vector<Type*>& vector2) {
  if (vector1.size() != vector2.size()) {
    return false;
  }
...
static std::string createValidPrefix(const std::vector<std::string>& fieldNames,
...
std::string Utils::calculateEnumFieldsPrefix(
    const std::string& enumName, const std::vector<std::string>& fields) {
  for (size_t prefixLength = 0; prefixLength < enumName.size();
       prefixLength++) {
    char c = enumName[prefixLength];
    for (size_t i = 0; i < fields.size(); i++) {
      if (prefixLength >= fields[i].size() || fields[i][prefixLength] != c) {
        while (prefixLength > 0 && (!std::isupper(fields[i][prefixLength]) ||
                                    !isalpha(fields, prefixLength))) {
          prefixLength--;
        }
...
void Utils::getAllLinkLibraries(
    clang::Module* module, std::vector<clang::Module::LinkLibrary>& result) {
  for (clang::Module::LinkLibrary lib : module->LinkLibraries)
    result.push_back(lib);
  for (clang::Module::submodule_const_iterator it =
           module->submodules().begin();

...
None. Every file is byte-identical to upstream.

Two build-level accommodations keep it that way:

- `js_native_api_v8.cc` compiles with `-Wno-deprecated-declarations
  -Wno-shorten-64-to-32` (per-file `COMPILER_FLAGS` in `v8ios.xcodeproj`).
  V8 14.9 marks `ClearWeak`, `Get/SetPrototypeV2` and
  `AdjustAmountOfExternalAllocatedMemory` `V8_DEPRECATE_SOON`, and upstream
  freely narrows `size_t` to `int`; the project's warning settings would
...
for f in js_native_api.h js_native_api_types.h js_native_api_v8.h js_native_api_v8.cc node_api.h node_api_types.h; do
  curl -sSfo "NativeScript/napi/vendor/$f" "https://raw.githubusercontent.com/nodejs/node/$V/src/$f"
done
```

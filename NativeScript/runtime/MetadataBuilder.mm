...
void MetadataBuilder::RegisterInstanceProperties(
  ...
  for (auto it = meta->instanceProps->begin(); it != meta->instanceProps->end(); it++) {
    ...
    if (accessors > 0 && (propertiesIt == names.end() || propertiesIt->second < accessors)) {
      ...
      Caches::Get(isolate)->registerCacheBoundObject(item);
      Local<External> ext = External::New(isolate, item);
      v8::FunctionCallback getterCb = PropertyGetterCallback;
      if (propMeta->hasGetter()) {
  ...
  if (desc.has_set()) {
    FFIMethodCallback setterCallback = [](ffi_cif* cif, void* retValue, void** argValues,
      ...
      Local<Object> self_ =
          it != cache->Instances.end()
              ? it->second->Get(context->isolate_).As<Object>()
              : ArgConverter::CreateJsWrapper(v8Context, new ObjCDataWrapper(thiz), Local<Object>())
                    .As<Object>();
      ...
      tns::Assert(
          setterFunc->Call(context->isolate_->GetCurrentContext(), self_, 1, params).ToLocal(&res),
          context->isolate_);
    };

...
ffi_type* FFICall::GetArgumentType(const TypeEncoding* typeEncoding,
  ...
  switch (typeEncoding->type) {
    ...
    case BinaryTypeEncodingType::ExtVectorEncoding: {
      ...
      const TypeEncoding* innerType =
          typeEncoding->details.extVector.getInnerType();
      ffi_type* innerFFIType =
          FFICall::GetArgumentType(innerType, isStructMember);
      ffi_type* type = new ffi_type({.size = size * innerFFIType->size,
                                     .alignment = innerFFIType->alignment,
                                     .type = FFI_TYPE_EXT_VECTOR});
    ...
    case BinaryTypeEncodingType::ConstantArrayEncoding: {
      if (isStructMember) {
        const TypeEncoding* innerType =
            typeEncoding->details.constantArray.getInnerType();
        ffi_type* innerFFIType =
            FFICall::GetArgumentType(innerType, isStructMember);
        int32_t size = typeEncoding->details.constantArray.size;
        ffi_type* ffiType = new ffi_type({.size = size * innerFFIType->size,
                                          .alignment = innerFFIType->alignment,
    ...
    case BinaryTypeEncodingType::AnonymousStructEncoding: {
      ...
      const String* fieldNames =
          typeEncoding->details.anonymousRecord.getFieldNames();
  ...
  switch (typeEncoding->type) {
    case BinaryTypeEncodingType::ExtVectorEncoding: {
      ...
      if (type->elements[0] != nullptr) {
        DisposeFFIType(type->elements[0],
                       typeEncoding->details.extVector.getInnerType());
      }
...
StructInfo FFICall::GetStructInfo(const StructMeta* structMeta,
  ...
  StructInfo structInfo = FFICall::GetStructInfo(fieldsCount, fieldEncoding,
                                                 fieldNames, structName);
  return structInfo;
}

StructInfo FFICall::GetStructInfo(size_t fieldsCount,
                                  const TypeEncoding* fieldEncoding,
                                  const String* fieldNames,
                                  std::string structName) {
  if (structName.empty()) {
...
#if defined(__x86_64__)
    hasNestedStruct =
        hasNestedStruct || (fieldFFIType->type == FFI_TYPE_STRUCT);
#endif
...
#if defined(__x86_64__)
  /*
   If on 64-bit architecture, flatten the nested structures, because libffi
   can't handle them.
   */
  if (hasNestedStruct) {
    std::vector<ffi_type*> flattenedFfiTypes;
    std::vector<ffi_type*>
        stack;  // simulate recursion with stack (no need of other function)
    ...
    while (!stack.empty()) {
      ...
      if (currentType->type != FFI_TYPE_STRUCT) {
        ...
        ffi_type** nullPtr =
            currentType->elements;  // the end of elements array
        while (*nullPtr != nullptr) {
          nullPtr++;
        }

        // add fields' ffi types in reverse order in the stack, so they will be
        // popped in correct order
        for (ffi_type** field = nullPtr - 1; field >= currentType->elements;
             field--) {
    ...
    memcpy(ffiType->elements, flattenedFfiTypes.data(),
           flattenedFfiTypes.size() * sizeof(ffi_type*));
...
ParametrizedCall* ParametrizedCall::Get(const TypeEncoding* typeEncoding,
  ...
  ffi_status status =
      ffi_prep_cif(cif, FFI_DEFAULT_ABI, argsCount, returnType,
                   const_cast<ffi_type**>(parameterTypesFFITypes));
...
robin_hood::unordered_map<const TypeEncoding*, ParametrizedCall*>
    ParametrizedCall::callsCache_;
robin_hood::unordered_map<std::string, StructInfo> FFICall::structInfosCache_;

StructInfo* FFICall::FindCachedStructInfo(const std::string& name) {
  auto it = structInfosCache_.find(name);
  if (it != structInfosCache_.end()) {
    return &it->second;
  }
  return nullptr;

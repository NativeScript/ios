...
void FunctionReference::Register(Local<Context> context,
                                 Local<Object> interop) {
  Isolate* isolate = context->GetIsolate();
  Local<v8::Function> ctorFunc =
      FunctionReference::GetFunctionReferenceCtorFunc(context);
  bool success =
      interop
          ->Set(context, tns::ToV8String(isolate, "FunctionReference"),
                ctorFunc)
          .FromMaybe(false);
  tns::Assert(success, isolate);
}

Local<v8::Function> FunctionReference::GetFunctionReferenceCtorFunc(
    Local<Context> context) {
  Isolate* isolate = context->GetIsolate();
  auto cache = Caches::Get(isolate);
  Persistent<v8::Function>* poFunctionReferenceCtor =
      cache->FunctionReferenceCtorFunc.get();
  if (poFunctionReferenceCtor != nullptr) {
    return poFunctionReferenceCtor->Get(isolate);
  }

  Local<FunctionTemplate> ctorFuncTemplate =
      FunctionTemplate::New(isolate, FunctionReferenceConstructorCallback);
  ...
  cache->FunctionReferenceCtorFunc =
      std::make_unique<Persistent<v8::Function>>(isolate, ctorFunc);
  cache->FunctionReferenceCtorFunc->SetWrapperClassId(
      Constants::ClassTypes::DataWrapper);

  return ctorFunc;
}

void FunctionReference::FunctionReferenceConstructorCallback(

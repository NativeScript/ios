...
void Pointer::Register(Local<Context> context, Local<Object> interop) {
  ...
  bool success =
      interop->Set(context, tns::ToV8String(isolate, "Pointer"), ctorFunc)
          .FromMaybe(false);
...
Local<Value> Pointer::NewInstance(Local<Context> context, void* handle) {
  ...
  Local<Value> result;
  Local<v8::Function> ctorFunc = Pointer::GetPointerCtorFunc(context);
  bool success = ctorFunc->NewInstance(isolate->GetCurrentContext(), 1, args)
                     .ToLocal(&result);
  if (!success) {
...
Local<v8::Function> Pointer::GetPointerCtorFunc(Local<Context> context) {
  ...
  Local<FunctionTemplate> ctorFuncTemplate =
      FunctionTemplate::New(isolate, PointerConstructorCallback);
...
void Pointer::PointerConstructorCallback(
  ...
  try {
    ...
    if (info.Length() == 1) {
      ...
      if (!isBigInt && !isNumber) {
        throw NativeScriptException(
            "Pointer constructor's first arg must be an integer.");
      }
      ...
      if (isBigInt) {
        ...
        // TODO: Maybe log this?
        //#ifdef DEBUG
        //                syslog(LOG_WARNING, "Using JS Number to represent a
        //                pointer. Result might be wrong.");
        //#endif
        tns::Assert(info[0].As<Number>()->IntegerValue(context).To(&value),
                    isolate);
      }
      ...
      if (isBigInt) {
        ...
        tns::Assert(info[0].As<Number>()->Int32Value(context).To(&value),
                    isolate);
      }
      ptr = reinterpret_cast<void*>(value);
#endif
    ...
    cache->PointerInstances.emplace(
        ptr, std::make_shared<Persistent<Object>>(isolate, info.This()));
  } catch (NativeScriptException& ex) {
    ex.ReThrowToV8(isolate);
  }
}

void Pointer::RegisterAddMethod(Local<Context> context,
  ...
  Local<FunctionTemplate> funcTemplate = FunctionTemplate::New(
      isolate, [](const FunctionCallbackInfo<Value>& info) {
        ...
        void* newValue =
            reinterpret_cast<void*>(reinterpret_cast<char*>(value) + offset);
  ...
  bool success = prototype->Set(context, tns::ToV8String(isolate, "add"), func)
                     .FromMaybe(false);
  tns::Assert(success, isolate);
}

void Pointer::RegisterSubtractMethod(Local<Context> context,
                                     Local<Object> prototype) {
  Isolate* isolate = context->GetIsolate();
  Local<FunctionTemplate> funcTemplate = FunctionTemplate::New(
      isolate, [](const FunctionCallbackInfo<Value>& info) {
        ...
        void* newValue =
            reinterpret_cast<void*>(reinterpret_cast<char*>(value) - offset);
        intptr_t newValuePtr =
            static_cast<intptr_t>(reinterpret_cast<size_t>(newValue));
  ...
  bool success =
      prototype->Set(context, tns::ToV8String(isolate, "subtract"), func)
          .FromMaybe(false);
  tns::Assert(success, isolate);
}

void Pointer::RegisterToStringMethod(Local<Context> context,
                                     Local<Object> prototype) {
  Isolate* isolate = context->GetIsolate();
  Local<FunctionTemplate> funcTemplate = FunctionTemplate::New(
      isolate, [](const FunctionCallbackInfo<Value>& info) {
...
void Pointer::RegisterToHexStringMethod(Local<Context> context,
                                        Local<Object> prototype) {
  Isolate* isolate = context->GetIsolate();
  Local<FunctionTemplate> funcTemplate = FunctionTemplate::New(
      isolate, [](const FunctionCallbackInfo<Value>& info) {
  ...
  bool success =
      prototype->Set(context, tns::ToV8String(isolate, "toHexString"), func)
          .FromMaybe(false);
  tns::Assert(success, isolate);
}

void Pointer::RegisterToDecimalStringMethod(Local<Context> context,
                                            Local<Object> prototype) {
  Isolate* isolate = context->GetIsolate();
  Local<FunctionTemplate> funcTemplate = FunctionTemplate::New(
      isolate, [](const FunctionCallbackInfo<Value>& info) {
  ...
  bool success =
      prototype->Set(context, tns::ToV8String(isolate, "toDecimalString"), func)
          .FromMaybe(false);
  tns::Assert(success, isolate);
}

void Pointer::RegisterToNumberMethod(Local<Context> context,
                                     Local<Object> prototype) {
  Isolate* isolate = context->GetIsolate();
  Local<FunctionTemplate> funcTemplate = FunctionTemplate::New(
      isolate, [](const FunctionCallbackInfo<Value>& info) {
  ...
  bool success =
      prototype->Set(context, tns::ToV8String(isolate, "toNumber"), func)
          .FromMaybe(false);
  tns::Assert(success, isolate);
}

void Pointer::RegisterToBigIntMethod(Local<Context> context,
                                     Local<Object> prototype) {
  Isolate* isolate = context->GetIsolate();
  Local<FunctionTemplate> funcTemplate = FunctionTemplate::New(
      isolate, [](const FunctionCallbackInfo<Value>& info) {
  ...
  bool success =
      prototype->Set(context, tns::ToV8String(isolate, "toBigInt"), func)
          .FromMaybe(false);
  tns::Assert(success, isolate);
}

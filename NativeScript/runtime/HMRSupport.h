...
// ─────────────────────────────────────────────────────────────
// Small v8 utility helpers (shared between Runtime.mm and HMRSupport.mm)
//
// These used to be duplicated as file-local statics in both translation
// units. Declared here once so the implementations live next to the HMR
// dev-session machinery that consumes them most heavily.
...
//
// Installs every JS-callable global the @nativescript/vite HMR client and
// the dev-session bootstrap depend on. Replaces ~650 lines of inline
// lambdas in Runtime::Init with a single call. Idempotent per realm; safe
// to call from any place that has a fresh context + isolate scope.
//

...
void URLSearchParamsImpl::Init(v8::Isolate* isolate,
...
v8::Local<v8::FunctionTemplate> URLSearchParamsImpl::GetCtor(
    v8::Isolate* isolate) {
  ...
  tmpl->Set(ToV8String(isolate, "append"),
            v8::FunctionTemplate::New(isolate, Append));
  tmpl->Set(ToV8String(isolate, "delete"),
  ...
  tmpl->Set(ToV8String(isolate, "entries"), entriesTmpl);

  tmpl->Set(ToV8String(isolate, "forEach"),
            v8::FunctionTemplate::New(isolate, ForEach));

  tmpl->Set(ToV8String(isolate, "get"),
            v8::FunctionTemplate::New(isolate, Get));

  tmpl->Set(ToV8String(isolate, "getAll"),
            v8::FunctionTemplate::New(isolate, GetAll));

  tmpl->Set(ToV8String(isolate, "has"),
            v8::FunctionTemplate::New(isolate, Has));

  tmpl->Set(ToV8String(isolate, "keys"),
...
// Spec-compliant URLSearchParams init (record / sequence / primitive) and the
// live iterators backing entries()/keys()/values()/@@iterator. The dispatch
// in Ctor mirrors the WebIDL union conversion to
// `(sequence<sequence<USVString>> or record<USVString, USVString> or
// USVString)` that browsers/Node apply
...
void IteratorNext(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (!state->Get(context, ToV8String(isolate, kStateSource))
           .ToLocal(&sourceValue) ||
      !state->Get(context, indexKey).ToLocal(&indexValue) ||
      !state->Get(context, ToV8String(isolate, kStateKind))
           .ToLocal(&kindValue)) {
    return;
  }
  ...
  auto valueKey = ToV8String(isolate, "value");
  auto doneKey = ToV8String(isolate, "done");

  URLSearchParamsImpl* ptr =
      sourceValue->IsObject()
          ? URLSearchParamsImpl::GetPointer(sourceValue.As<v8::Object>())
          : nullptr;
  ...
  if (kind == ITER_KEYS) {
    value = ToV8String(isolate, pair.first);
  } else if (kind == ITER_VALUES) {
    value = ToV8String(isolate, pair.second);
  } else {
    v8::Local<v8::Value> kv[] = {
        ToV8String(isolate, pair.first),
        ToV8String(isolate, pair.second),
    };
    value = v8::Array::New(isolate, kv, 2);
  }
  state
      ->Set(context, indexKey, v8::Integer::NewFromUnsigned(isolate, index + 1))
      .Check();
...
void ReturnLiveIterator(const v8::FunctionCallbackInfo<v8::Value>& args,
                        int kind) {
  auto isolate = args.GetIsolate();
  auto context = isolate->GetCurrentContext();
  ...
  state
      ->Set(context, ToV8String(isolate, kStateIndex),
            v8::Integer::NewFromUnsigned(isolate, 0))
      .Check();
  state
      ->Set(context, ToV8String(isolate, kStateKind),
  ...
  iterator
      ->DefineOwnProperty(context, v8::Symbol::GetToStringTag(isolate),
                          ToV8String(isolate, "URLSearchParams Iterator"),
                          v8::PropertyAttribute::DontEnum)
      .Check();
...
bool OpenIterator(v8::Local<v8::Context> context, v8::Local<v8::Value> receiver,
  ...
  if (!outIterator->Get(context, ToV8String(isolate, "next"))
           .ToLocal(&nextValue)) {
    return false;
  }
...
bool StepIterator(v8::Local<v8::Context> context,
  ...
  if (!result->Get(context, ToV8String(isolate, "done")).ToLocal(&doneValue)) {
    return false;
  }
  ...
  return result->Get(context, ToV8String(isolate, "value")).ToLocal(&outValue);
}
...
void CloseIterator(v8::Local<v8::Context> context,
  ...
  if (iterator->Get(context, ToV8String(isolate, "return"))
          .ToLocal(&returnMethod) &&
      returnMethod->IsFunction()) {
    v8::Local<v8::Value> ignored;
    (void)returnMethod.As<v8::Function>()
...
bool MaterializeInnerSequence(v8::Local<v8::Context> context,
  ...
  if (!value->IsObject()) {
    ThrowTypeError(isolate,
                   "URLSearchParams init sequence element is not iterable");
    return false;
  }
  ...
  if (!OpenIterator(context, object, iterMethod.As<v8::Function>(), iterator,
                    next)) {
    return false;
  }
  while (true) {
...
bool BuildFromSequence(v8::Local<v8::Context> context,
                       v8::Local<v8::Object> object,
                       v8::Local<v8::Function> iterMethod,
...
void URLSearchParamsImpl::Ctor(
  ...
  if (value->IsString()) {
    ...
    params =
        ada::url_search_params(tns::ToString(isolate, value.As<v8::String>()));
  }
  ...
  // A failed coercion leaves a pending exception; return so V8 throws it.
    ...
    if (iterMethod->IsNullOrUndefined()) {
      ...
      if (!BuildFromSequence(context, object, iterMethod.As<v8::Function>(),
                             params)) {
        return;
      }
    } else {
...
void URLSearchParamsImpl::Append(
    const v8::FunctionCallbackInfo<v8::Value>& args) {
...
void URLSearchParamsImpl::Delete(
  ...
  // or throwing toString leaves the pending exception and aborts).
  ...
  if (args.Length() > 1 && !args[1]->IsUndefined()) {
    std::string value;
    if (!ValueToString(isolate->GetCurrentContext(), args[1], value)) {
...
void URLSearchParamsImpl::Entries(
    const v8::FunctionCallbackInfo<v8::Value>& args) {
...
void URLSearchParamsImpl::ForEach(
  ...
  auto thisArg =
      args.Length() > 1 ? args[1] : v8::Undefined(isolate).As<v8::Value>();
...
void URLSearchParamsImpl::GetAll(
    const v8::FunctionCallbackInfo<v8::Value>& args) {
  URLSearchParamsImpl* ptr = GetPointer(args.This());
  auto isolate = args.GetIsolate();
  auto context = isolate->GetCurrentContext();
  ...
  for (auto item : values) {
    tns::Assert(
        ret->Set(context, i++, ToV8String(isolate, item)).FromMaybe(false),
        isolate);
  }
  args.GetReturnValue().Set(ret);
}
...
void URLSearchParamsImpl::ToString(
    const v8::FunctionCallbackInfo<v8::Value>& args) {
  URLSearchParamsImpl* ptr = GetPointer(args.This());
  if (ptr == nullptr) {
    ThrowTypeError(args.GetIsolate(), "Illegal invocation");
...
// Live ES iterator (see Entries above).
void URLSearchParamsImpl::Values(
    const v8::FunctionCallbackInfo<v8::Value>& args) {
...
NODE_BINDING_PER_ISOLATE_INIT_OBJ(urlsearchparams,
                                  tns::URLSearchParamsImpl::Init)
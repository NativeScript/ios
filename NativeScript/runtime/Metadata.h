...
template <typename V>
static const V& getProperFunctionFromContainer(
    const std::vector<V>& container, int argsCount,
    std::function<int(const V&)> paramsCounter) {
  ...
  for (const V& func : container) {
    ...
    } else if (argsCount < candidateArgs &&
               (calleeArgs < argsCount || candidateArgs < calleeArgs)) {
      // better candidate - looking for the least number of arguments which is
      // more than the amount actually passed
      callee = &func;
      calleeArgs = candidateArgs;
    } else if (calleeArgs < candidateArgs) {
      // better candidate - looking for the maximum number of arguments which
      // less than the amount actually passed (if one with more cannot be found)
      callee = &func;
      calleeArgs = candidateArgs;
    }
  }
...
struct Array {
  class iterator {
   ...
   public:
    iterator(const T* item) : current(item) {}
  ...
  const T* first() const { return reinterpret_cast<const T*>(&count + 1); }

  const T& operator[](int index) const { return *(first() + index); }

  Array<T>::iterator begin() const { return first(); }

  Array<T>::iterator end() const { return first() + count; }
  ...
  int sizeInBytes() const { return sizeof(Array<T>) + sizeof(T) * count; }
...
struct GlobalTable {
  class iterator {
   ...
   public:
    ...
    iterator(const GlobalTable<TYPE>* globalTable, int32_t topLevelIndex,
             int32_t bucketIndex)
  ...
  iterator end() const { return iterator(this, this->buckets.count, 0); }

  ArrayOfPtrTo<ArrayOfPtrTo<Meta>> buckets;

  const InterfaceMeta* findInterfaceMeta(const char* identifierString) const;

  const InterfaceMeta* findInterfaceMeta(const char* identifierString,
                                         size_t length, unsigned hash) const;

  const ProtocolMeta* findProtocol(const char* identifierString) const;

  const ProtocolMeta* findProtocol(const char* identifierString, size_t length,
                                   unsigned hash) const;

  const Meta* findMeta(const char* identifierString,
                       bool onlyIfAvailable = true) const;

  const Meta* findMeta(const char* identifierString, size_t length,
                       unsigned hash, bool onlyIfAvailable = true) const;

  int sizeInBytes() const { return buckets.sizeInBytes(); }

  static bool compareName(const Meta& meta, const char* identifierString,
                          size_t length);
};
...
struct ModuleTable {
  ...
  int sizeInBytes() const { return modules.sizeInBytes(); }
};
...
struct MetaFile {
 ...
 public:
  ...
  const GlobalTable<GlobalTableType::ByNativeName>* globalTableNativeProtocols()
      const {
    const GlobalTable<GlobalTableType::ByJsName>* gt = this->globalTableJs();
    return reinterpret_cast<const GlobalTable<GlobalTableType::ByNativeName>*>(
        offset(gt, gt->sizeInBytes()));
  }

  const GlobalTable<GlobalTableType::ByNativeName>*
  globalTableNativeInterfaces() const {
    const GlobalTable<GlobalTableType::ByNativeName>* gt =
        this->globalTableNativeProtocols();
    return reinterpret_cast<const GlobalTable<GlobalTableType::ByNativeName>*>(
        offset(gt, gt->sizeInBytes()));
  }
  ...
  const ModuleTable* topLevelModulesTable() const {
    const GlobalTable<GlobalTableType::ByNativeName>* gt =
        this->globalTableNativeInterfaces();
...
struct PtrTo {
  ...
  inline bool isNull() const { return offset == 0; }
  inline PtrTo<T> operator+(int value) const { return add(value); }
  inline const T* operator->() const { return valuePtr(); }
  ...
  inline const T* valuePtr() const {
    return isNull() ? nullptr
                    : reinterpret_cast<const T*>(tns::offset(
                          MetaFile::instance()->heap(), this->offset));
  }
  inline const T& value() const { return *valuePtr(); }
};
...
union TypeEncodingDetails {
  ...
  struct AnonymousRecordDetails {
    ...
    const TypeEncoding* getFieldsEncodings() const {
      return reinterpret_cast<const TypeEncoding*>(getFieldNames() +
                                                   this->fieldsCount);
    }
  } anonymousRecord;
};
...
struct TypeEncoding {
  ...
  const TypeEncoding* next() const {
    const TypeEncoding* afterTypePtr =
        reinterpret_cast<const TypeEncoding*>(offset(this, sizeof(type)));

    switch (this->type) {
      case BinaryTypeEncodingType::IdEncoding: {
        return reinterpret_cast<const TypeEncoding*>(
            offset(afterTypePtr, sizeof(TypeEncodingDetails::IdDetails)));
      }
      ...
      case BinaryTypeEncodingType::FunctionPointerEncoding: {
        const TypeEncoding* current =
            this->details.functionPointer.signature.first();
        for (int i = 0; i < this->details.functionPointer.signature.count;
             i++) {
          current = current->next();
        }
        return current;
...
struct ModuleMeta {
 public:
  ...
  const char* getName() const { return name.valuePtr(); }

  bool isFramework() const { return (flags & 1) > 0; }

  bool isSystem() const { return (flags & 2) > 0; }
};
...
struct Meta {
 ...
 public:
  ...
  inline bool hasName() const { return this->flag(MetaFlags::HasName); }
  ...
  inline const char* jsName() const { return this->getNameByIndex(JsName); }

  inline const char* name() const { return this->getNameByIndex(Name); }
   ...
   */
  inline uint8_t introducedIn() const { return this->_introduced; }
...
struct RecordMeta : Meta {
 ...
 public:
  ...
  inline size_t fieldsCount() const { return fieldNames().count; }
...
struct FunctionMeta : Meta {
 ...
 public:
  bool isVariadic() const { return this->flag(MetaFlags::FunctionIsVariadic); }
...
struct JsCodeMeta : Meta {
 ...
 public:
  inline const char* jsCode() const { return _jsCode.valuePtr(); }
};
...
struct VarMeta : Meta {
 ...
 public:
  inline const TypeEncoding* encoding() const { return _encoding.valuePtr(); }
};
...
struct MethodMeta : MemberMeta {
 ...
 public:
  ...
  inline SEL selector() const {
    static robin_hood::unordered_map<const MethodMeta*, SEL>
        methodMetaSelectorCache;
    // this method takes a few ns to run and is almost never called by another
    // thread this means that locking a mutex is almost always unecessary so we
    // use a spinlock that will most likely never spin
    static SpinMutex p;
    SpinLock lock(p);
    SEL ret = nullptr;
  ...
  inline const char* selectorAsString() const { return this->name(); }
  ...
  inline bool isAvailableInClasses(KnownUnknownClassPair klasses,
                                   bool isStatic) const {
    return this->isAvailableInClass(klasses.known, isStatic) ||
           (klasses.unknown != nullptr &&
            this->isAvailableInClass(klasses.unknown, isStatic));
  }
};
 ...
 public:
  ...
  inline const MethodMeta* setter() const {
    return (this->hasSetter())
               ? (this->hasGetter() ? method2.valuePtr() : method1.valuePtr())
               : nullptr;
  }

  inline bool isImplementedInClass(Class klass, bool isStatic) const {
    bool getterAvailable =
        this->hasGetter() &&
        this->getter()->isImplementedInClass(klass, isStatic);
    bool setterAvailable =
        this->hasSetter() &&
        this->setter()->isImplementedInClass(klass, isStatic);
...
struct BaseClassMeta : Meta {
  ...
  template <typename T>
  void forEachProtocol(const T& fun,
                       const ProtocolMetas* additionalProtocols) const {
    for (Array<String>::iterator it = this->protocols->begin();
         it != this->protocols->end(); ++it) {
      if (const ProtocolMeta* protocolMeta =
              MetaFile::instance()->globalTableJs()->findProtocol(
                  (*it).valuePtr())) {
        fun(protocolMeta);
      }
    }
  ...
  const MemberMeta* member(const char* identifier, size_t length,
                           MemberType type, bool includeProtocols,
                           bool onlyIfAvailable,
                           const ProtocolMetas& additionalProtocols) const;

  const MethodMeta* member(const char* identifier, size_t length,
                           MemberType type, size_t paramsCount,
                           bool includeProtocols, bool onlyIfAvailable,
                           const ProtocolMetas& additionalProtocols) const;

  const MembersCollection members(
      const char* identifier, size_t length, MemberType type,
      bool includeProtocols, bool onlyIfAvailable,
      const ProtocolMetas& additionalProtocols) const;

  const MemberMeta* member(const char* identifier, MemberType type,
                           bool includeProtocols,
                           const ProtocolMetas& additionalProtocols) const {
    return this->member(identifier, strlen(identifier), type, includeProtocols,
                        /*onlyIfAvailable*/ true, additionalProtocols);
  }

  /// instance methods

  // Remove all optional methods/properties which are not implemented in the
  // class
  template <typename TMemberMeta>
  static void filterUnavailableMembers(MembersCollection& members,
                                       KnownUnknownClassPair klasses,
                                       bool isStatic) {
    for (auto it{members.begin()}, end{members.end()}; it != end;) {
      const MemberMeta* memberMeta = *it;
      bool isAvailable = static_cast<const TMemberMeta*>(memberMeta)
                             ->isAvailableInClasses(klasses, isStatic);
      if (!isAvailable) {
        it = members.erase(it);
      } else {
        ++it;
  ...
  /// instance properties
  const PropertyMeta* instanceProperty(
      const char* identifier, KnownUnknownClassPair klasses,
      bool includeProtocols, const ProtocolMetas& additionalProtocols) const {
    auto propMeta = static_cast<const PropertyMeta*>(
        this->member(identifier, MemberType::InstanceProperty, includeProtocols,
                     additionalProtocols));
    return propMeta &&
                   propMeta->isAvailableInClasses(klasses, /*isStatic*/ false)
               ? propMeta
               : nullptr;
  }

  /// static properties
  const PropertyMeta* staticProperty(
      const char* identifier, KnownUnknownClassPair klasses,
      bool includeProtocols, const ProtocolMetas& additionalProtocols) const {
    auto propMeta = static_cast<const PropertyMeta*>(
        this->member(identifier, MemberType::StaticProperty, includeProtocols,
                     additionalProtocols));
    return propMeta &&
                   propMeta->isAvailableInClasses(klasses, /*isStatic*/ true)
               ? propMeta
               : nullptr;
  }

  /// vectors
  std::vector<const PropertyMeta*> instanceProperties(
  ...
  std::vector<const PropertyMeta*> instancePropertiesWithProtocols(
      KnownUnknownClassPair klasses,
      const ProtocolMetas& additionalProtocols) const {
    std::vector<const PropertyMeta*> properties;
    return this->instancePropertiesWithProtocols(properties, klasses,
                                                 additionalProtocols);
  }

  std::vector<const PropertyMeta*> instanceProperties(
      std::vector<const PropertyMeta*>& container,
      KnownUnknownClassPair klasses) const {
    for (Array<PtrTo<PropertyMeta>>::iterator it = this->instanceProps->begin();
         it != this->instanceProps->end(); it++) {
  ...
  std::vector<const PropertyMeta*> staticProperties(
      KnownUnknownClassPair klasses) const {
    std::vector<const PropertyMeta*> properties;
    return this->staticProperties(properties, klasses);
  }

  std::vector<const PropertyMeta*> staticPropertiesWithProtocols(
      KnownUnknownClassPair klasses,
      const ProtocolMetas& additionalProtocols) const {
    std::vector<const PropertyMeta*> properties;
    return this->staticPropertiesWithProtocols(properties, klasses,
                                               additionalProtocols);
  }
  ...
  std::vector<const PropertyMeta*> staticProperties(
      std::vector<const PropertyMeta*>& container,
      KnownUnknownClassPair klasses) const {
    for (Array<PtrTo<PropertyMeta>>::iterator it = this->staticProps->begin();
         it != this->staticProps->end(); it++) {
  ...
  std::vector<const MethodMeta*> initializersWithProtocols(
      KnownUnknownClassPair klasses,
      const ProtocolMetas& additionalProtocols) const {
    std::vector<const MethodMeta*> initializers;
    return this->initializersWithProtocols(initializers, klasses,
                                           additionalProtocols);
  }

  std::vector<const MethodMeta*> initializers(
      std::vector<const MethodMeta*>& container,
      KnownUnknownClassPair klasses) const;

  std::vector<const MethodMeta*> initializersWithProtocols(
      std::vector<const MethodMeta*>& container, KnownUnknownClassPair klasses,
      const ProtocolMetas& additionalProtocols) const;
};
...
struct InterfaceMeta : BaseClassMeta {
 ...
 public:
  const char* baseName() const { return _baseName.valuePtr(); }

  const InterfaceMeta* baseMeta() const {
    if (this->baseName() != nullptr) {
      const InterfaceMeta* baseMeta =
          MetaFile::instance()->globalTableJs()->findInterfaceMeta(
              this->baseName());
      return baseMeta;
    }

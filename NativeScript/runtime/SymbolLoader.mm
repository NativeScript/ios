...
class CFBundleSymbolResolver : public SymbolResolver {
 public:
  ...
  virtual void* loadFunctionSymbol(const char* symbolName) override {
    CFStringRef cfName = CFStringCreateWithCStringNoCopy(kCFAllocatorDefault, symbolName,
...
class DlSymbolResolver : public SymbolResolver {
 public:
  ...
  virtual bool load() override { return !!this->_libraryHandle; }
...
SymbolResolver* SymbolLoader::resolveModule(const ModuleMeta* module) {
  ...
  if (module->isFramework()) {
    ...
    if (module->isSystem()) {
      ...
      NSString* basePathStr = [foundationPath
          substringToIndex:[foundationPath rangeOfString:@"Foundation.framework"].location];
    ...
    if (module->isSystem()) {
      ...
      NSString* libraryPath =
          [NSString stringWithFormat:@"%@/lib%s.dylib", libsPath,

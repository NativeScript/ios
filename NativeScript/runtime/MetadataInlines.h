...
inline size_t compareIdentifiers(const char* nullTerminated,
                                 const char* notNullTerminated, size_t length) {
...
const InterfaceMeta* GlobalTable<TYPE>::findInterfaceMeta(
    const char* identifierString) const {
  unsigned hash = WTF::StringHasher::computeHashAndMaskTop8Bits<LChar>(
      reinterpret_cast<const LChar*>(identifierString));
  return this->findInterfaceMeta(identifierString, strlen(identifierString),
                                 hash);
}

template <GlobalTableType TYPE>
const InterfaceMeta* GlobalTable<TYPE>::findInterfaceMeta(
    const char* identifierString, size_t length, unsigned hash) const {
  const Meta* meta =
      this->findMeta(identifierString, length, hash, /*onlyIfAvailable*/ false);
  if (meta == nullptr) {
    return nullptr;
  }
  ...
  ASSERT(meta->type() == MetaType::Interface ||
         (meta->type() == MetaType::ProtocolType &&
          objc_getClass(meta->name()) != nullptr &&
          objc_getProtocol(meta->name()) != nullptr));
  ...
  if (interfaceMeta->isAvailable()) {
    ...
    tns::LogMetadataUnavailable(std::string(identifierString, length).c_str(),
...
template <GlobalTableType TYPE>
const ProtocolMeta* GlobalTable<TYPE>::findProtocol(
    const char* identifierString) const {
  unsigned hash = WTF::StringHasher::computeHashAndMaskTop8Bits<LChar>(
      reinterpret_cast<const LChar*>(identifierString));
...
const ProtocolMeta* GlobalTable<TYPE>::findProtocol(
    const char* identifierString, size_t length, unsigned hash) const {
  // Do not check for availability when returning a protocol. Apple regularly
  // create new protocols and move existing interface members there (e.g.
  // iOS 12.0 introduced the UIFocusItemScrollableContainer protocol in UIKit
  // which contained members that have existed in UIScrollView since iOS 2.0)
...
template <GlobalTableType TYPE>
const Meta* GlobalTable<TYPE>::findMeta(const char* identifierString,
                                        bool onlyIfAvailable) const {
  unsigned hash = WTF::StringHasher::computeHashAndMaskTop8Bits<LChar>(
      reinterpret_cast<const LChar*>(identifierString));
...
const Meta* GlobalTable<TYPE>::findMeta(const char* identifierString,
  ...
  for (ArrayOfPtrTo<Meta>::iterator it = bucketContent.begin();
       it != bucketContent.end(); it++) {
...
template <>
inline bool GlobalTable<ByJsName>::compareName(const Meta& meta,
...
template <>
inline bool GlobalTable<ByNativeName>::compareName(const Meta& meta,
                                                   const char* identifierString,
                                                   size_t length) {
  return compareIdentifiers(meta.name(), identifierString, length) == 0 ||
         (meta.hasDemangledName() &&
          compareIdentifiers(meta.demangledName(), identifierString, length) ==
              0);
}
...
const Meta* GlobalTable<TYPE>::iterator::getCurrent() {
  return this->_globalTable->buckets[_topLevelIndex]
      .value()[_bucketIndex]
      .valuePtr();
}

template <GlobalTableType TYPE>
typename GlobalTable<TYPE>::iterator&
GlobalTable<TYPE>::iterator::operator++() {
  this->_bucketIndex++;
  this->findNext();
  return *this;
}
...
void GlobalTable<TYPE>::iterator::findNext() {
  ...
  do {
    if (!this->_globalTable->buckets[_topLevelIndex].isNull()) {
      int bucketLength =
          this->_globalTable->buckets[_topLevelIndex].value().count;

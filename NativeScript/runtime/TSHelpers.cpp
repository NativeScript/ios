...
void TSHelpers::Init(Local<Context> context) {
  ...
  std::string source =
      ...
      "        var extendingNativeClass = !!Parent.extend && "
      "(Parent.extend.toString().indexOf(\"[native code]\") > -1);"
      ...
      "            throw new Error(\"Can not extend an already extended native "
      "object.\");"
      "        }"
      ""
      ...
      "                thiz.__proto__ = new "
      "(Function.prototype.bind.apply(Extended, "
      ...
      "                thiz.__proto__ = new "
      "(Function.prototype.bind.apply(Extended, [null].concat(args)));"
      "            } else {"
      "                thiz.__proto__ = new Extended();"
      "            }"
      "            return thiz.__proto__;"
      ...
      "        || (hasInternalProtoProperty() && function (child, parent) { "
      "child.__proto__ = parent; })"
  ...
  Local<Script> script;
  TryCatch tc(isolate);
  if (!Script::Compile(context, tns::ToV8String(isolate, source.c_str()))
           .ToLocal(&script) &&
      tc.HasCaught()) {

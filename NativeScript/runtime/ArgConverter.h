...
struct MethodCallbackWrapper {
 public:
  MethodCallbackWrapper(v8::Isolate* isolate,
                        std::shared_ptr<v8::Persistent<v8::Value>> callback,
                        const uint8_t initialParamIndex,
                        const uint8_t paramsCount,
...
class ArgConverter {
 public:
  ...
  static v8::Local<v8::Value> CreateJsWrapper(
      v8::Local<v8::Context> context, BaseDataWrapper* wrapper,
      v8::Local<v8::Object> receiver, bool skipGCRegistration = false,
      const std::vector<std::string>& additionalProtocols =
          std::vector<std::string>());
  ...
  static v8::Local<v8::Private> GetEscapeExceptionBrand(v8::Isolate* isolate);
  // If `value` carries the escapeException brand, returns the NSException to
  // throw (typed `id` so this header is usable from the pure-C++ TUs that
  // include it) — the original wrapped NSException when present, otherwise one
  // synthesized from the branded name/message/stack payload. Returns nil when
  // `value` is not branded. Never reports; the caller decides what to do.
  static id ExtractEscapedException(v8::Local<v8::Context> context,
                                    v8::Local<v8::Value> value);
  // Shared JS<->native boundary helper. Reads tc.Exception(): if it carries
  // the escapeException brand, returns the NSException (as `id`) the caller
  // must @throw AFTER closing every live V8 scope (Locker/HandleScope/
  // Context::Scope). Otherwise reports the exception exactly once through the
  // uncaught path (error-event dispatch + shims + log) and returns nil, so the
  // caller returns its defined default. Resets `tc` in both cases.
  static id HandleBoundaryException(v8::Local<v8::Context> context,
                                    v8::TryCatch& tc);
  static void ConstructObject(v8::Local<v8::Context> context,
                              const v8::FunctionCallbackInfo<v8::Value>& info,
                              Class klass,
                              const InterfaceMeta* interfaceMeta = nullptr);
 ...
 private:
  ...
  static std::shared_ptr<v8::Persistent<v8::Value>> CreateEmptyInstance(
      ...
      bool skipGCRegistration = false);
  static void FindMethodOverloads(Class klass, std::string methodName,
                                  MemberType type,
                                  std::vector<const MethodMeta*>& overloads);
  static const MethodMeta* FindInitializer(
      v8::Local<v8::Context> context, Class klass,
      const InterfaceMeta* interfaceMeta,
      const v8::FunctionCallbackInfo<v8::Value>& info,
      std::vector<v8::Local<v8::Value>>& args);
  static bool CanInvoke(v8::Local<v8::Context> context,
                        const TypeEncoding* typeEncoding,
                        v8::Local<v8::Value> arg);
  static bool CanInvoke(v8::Local<v8::Context> context,
                        const MethodMeta* candidate,
                        const v8::FunctionCallbackInfo<v8::Value>& info);
  ...
  static void MethodCallbackInternal(ffi_cif* cif, void* retValue,

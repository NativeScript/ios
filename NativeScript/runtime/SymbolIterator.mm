...
void SymbolIterator::NextCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  size_t index =
      (size_t)thiz->GetAlignedPointerFromInternalField(1, v8::kEmbedderDataTypeTagDefault) >> 1;

  Local<Value> object = thiz->GetInternalField(0).As<v8::Value>();
  ...
  if (index >= [target count]) {
    bool success =
        obj->Set(context, tns::ToV8String(isolate, "done"), v8::Boolean::New(isolate, true))
            .FromMaybe(false);
    tns::Assert(success, isolate);
    success = obj->Set(context, tns::ToV8String(isolate, "value"), v8::Undefined(isolate))
                  .FromMaybe(false);
    tns::Assert(success, isolate);
  } else {
    ...
    bool success =
        obj->Set(context, tns::ToV8String(isolate, "done"), v8::Boolean::New(isolate, false))
            .FromMaybe(false);
    tns::Assert(success, isolate);

...
Local<v8::Function> Reference::GetInteropReferenceCtorFunc(
  ...
  proto->SetAccessor(tns::ToV8String(isolate, "value"), GetValueCallback,
                     SetValueCallback);
...
void Reference::IndexedPropertyGetCallback(
...
void Reference::IndexedPropertySetCallback(
...
void Reference::GetValueCallback(Local<v8::Name> name,
                                 const PropertyCallbackInfo<Value>& info) {
  Isolate* isolate = info.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
...
void Reference::SetValueCallback(Local<v8::Name> name, Local<Value> value,
...
Local<Value> Reference::GetReferredValue(Local<Context> context,
  ...
  if (typeWrapper != nullptr && typeWrapper->Type() == WrapperType::Primitive &&
    ...
    if (pair.data_ != nullptr && pair.typeEncoding_ != nullptr) {
      ...
      Local<Value> result =
          Interop::GetResult(context, pair.typeEncoding_, &call, false);
      return result;
    }
  }
...
Reference::DataPair Reference::GetTypeEncodingDataPair(Local<Object> obj) {
  ...
  if (typeWrapper->Type() != WrapperType::Primitive) {
    // TODO: Currently only PrimitiveDataWrappers are supported as type
    // parameters Objective C class classes and structures should also be
    // handled
    tns::Assert(false, isolate);
  }
  ...
  if (refWrapper->Encoding() != nullptr && refWrapper->Data() != nullptr) {
    DataPair pair(refWrapper->Encoding(), refWrapper->Data(),
                  primitiveWrapper->Size());
    return pair;
  }

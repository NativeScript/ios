...
CFTypeRef Interop::CreateBlock(const uint8_t initialParamIndex, const uint8_t argsCount,
  ...
  AOTBlockInvokeFunc aotInvoke = GetAOTBlockInvoke(typeEncoding, argsCount);
  ffi_closure* closure = nullptr;
  const void* invokePtr = nullptr;

  if (aotInvoke) {
    invokePtr = aotInvoke;
  } else {
    std::pair<IMP, ffi_closure*> result = Interop::CreateMethodInternal(
        initialParamIndex, argsCount, typeEncoding, callback, userData);
    invokePtr = (const void*)result.first;
    closure = result.second;
  }
  ...
  if (arg.IsEmpty() || arg->IsNullOrUndefined()) {
    ...
    if (arg->IsArrayBuffer()) {
      ...
      std::shared_ptr<BackingStore> backingStore =
          arg.As<ArrayBufferView>()->Buffer()->GetBackingStore();
      data = backingStore->Data();
    } else {
...
void ExecuteWriteValueValidationsAndStopExecutionAndLogStackTrace(Local<Context> context,
  ...
  if (typeEncoding->type == BinaryTypeEncodingType::InterfaceDeclarationReference) {
    if (originArg->IsObject()) {
      ...
      if ((originObj->IsArrayBuffer() || originObj->IsArrayBufferView()) && destName != "NSArray") {

...
Local<Value> UnmanagedType::Create(Local<Context> context, UnmanagedTypeWrapper* wrapper) {
  ...
  if (cache->UnmanagedTypeCtorFunc.get() == nullptr) {
    ...
    Local<FunctionTemplate> takeUnretainedValueFuncTemplate =
        FunctionTemplate::New(isolate, UnmanagedType::TakeUnretainedValueCallback);
    Local<FunctionTemplate> takeRetainedValueFuncTemplate =
        FunctionTemplate::New(isolate, UnmanagedType::TakeRetainedValueCallback);
...
void UnmanagedType::ConstructorCallback(const FunctionCallbackInfo<Value>& info) {
  ...
  UnmanagedTypeWrapper* wrapper =
      static_cast<UnmanagedTypeWrapper*>(ext->Value(v8::kExternalPointerTypeTagDefault));

...
Instrumentation& Runtime::instrumentation() {
  class NoInstrumentation : public Instrumentation {
    ...
    void createSnapshotToStream(
        std::ostream& /*os*/, const HeapSnapshotOptions& /*options*/) override {
      throw JSINativeException(
          "Default instrumentation cannot create a heap snapshot");
    }

    std::string flushAndDisableBridgeTrafficTrace() override { std::abort(); }
...
void Runtime::getStringData(const jsi::String& str, void* ctx,
...
void Runtime::getPropNameIdData(const jsi::PropNameID& sym, void* ctx,
...
Function Object::getPropertyAsFunction(Runtime& runtime,
...
bool Value::strictEquals(Runtime& runtime, const Value& a, const Value& b) {
  ...
  switch (a.kind_) {
    ...
    case BigIntKind:
      return runtime.strictEquals(static_cast<const BigInt&>(a.data_.pointer),
...
bool Value::asBool() const {
  if (!isBool()) {
    throw JSINativeException("Value is " + kindToString(*this) +
                             ", expected a boolean");
  }
...
Array Array::createWithElements(Runtime& rt,
...
JSError::JSError(Runtime& rt, std::string msg) : message_(std::move(msg)) {
  try {
    setValue(rt, callGlobalFunction(rt, "Error",
                                    String::createFromUtf8(rt, message_)));

...
class DecoratedHostFunction {
 public:
  ...
  Runtime& decoratedRuntime() { return drt_; }

  Value operator()(Runtime&, const Value& thisVal, const Value* args,
                   size_t count) {
...
class RuntimeDecorator : public Base, private jsi::Instrumentation {
 public:
  Plain& plain() {
    ...
    static_assert(std::is_base_of<Runtime, Base>::value,
                  "RuntimeDecorator's Base type must derive from jsi::Runtime");
    return plain_;
  }
  const Plain& plain() const { return plain_; }
  ...
  Object global() override { return plain().global(); }
  std::string description() override { return plain().description(); };
  bool isInspectable() override { return plain().isInspectable(); };
  Instrumentation& instrumentation() override { return *this; }
 ...
 protected:
  ...
  std::string utf8(const PropNameID& id) override { return plain_.utf8(id); };
  ...
  uint64_t truncate(const BigInt& b) override { return plain_.truncate(b); }
  ...
  std::string utf8(const String& s) override { return plain_.utf8(s); }

  std::u16string utf16(const String& str) override { return plain_.utf16(str); }
  std::u16string utf16(const PropNameID& sym) override {
    return plain_.utf16(sym);
  }

  void getStringData(const jsi::String& str, void* ctx,
                     void (*cb)(void* ctx, bool ascii, const void* data,
                                size_t num)) override {
    plain_.getStringData(str, ctx, cb);
  }

  void getPropNameIdData(const jsi::PropNameID& sym, void* ctx,
                         void (*cb)(void* ctx, bool ascii, const void* data,
                                    size_t num)) override {
  ...
  Object createObject() override { return plain_.createObject(); };
  ...
  void setNativeState(const Object& o,
  ...
  void setPropertyValue(const Object& o, const PropNameID& name,
                        const Value& value) override {
    plain_.setPropertyValue(o, name, value);
  };
  void setPropertyValue(const Object& o, const String& name,
                        const Value& value) override {
    plain_.setPropertyValue(o, name, value);
  };

  bool isArray(const Object& o) const override { return plain_.isArray(o); };
  ...
  size_t size(const Array& a) override { return plain_.size(a); };
  size_t size(const ArrayBuffer& ab) override { return plain_.size(ab); };
  uint8_t* data(const ArrayBuffer& ab) override { return plain_.data(ab); };
  Value getValueAtIndex(const Array& a, size_t i) override {
    return plain_.getValueAtIndex(a, i);
  };
  void setValueAtIndexImpl(const Array& a, size_t i,
                           const Value& value) override {
  ...
  Value call(const Function& f, const Value& jsThis, const Value* args,
             size_t count) override {
    return plain_.call(f, jsThis, args, count);
  };
  Value callAsConstructor(const Function& f, const Value* args,
                          size_t count) override {
    return plain_.callAsConstructor(f, args, count);
  };

  // Private data for managing scopes.
  Runtime::ScopeState* pushScope() override { return plain_.pushScope(); }
  void popScope(Runtime::ScopeState* ss) override { plain_.popScope(ss); }
...
struct BeforeCaller<T, decltype((void)&T::before)> {
  static void before(T& t) { t.before(); }
};

template <typename T>
struct AfterCaller<T, decltype((void)&T::after)> {
  static void after(T& t) { t.after(); }
};
...
struct BeforeCaller<std::tuple<T...>> {
  static void before(std::tuple<T...>& tuple) { all_before<0, T...>(tuple); }
...
struct AfterCaller<std::tuple<T...>> {
  static void after(std::tuple<T...>& tuple) { all_after<0, T...>(tuple); }
...
class WithRuntimeDecorator : public RuntimeDecorator<Plain, Base> {
 ...
 protected:
  ...
  Function createFunctionFromHostFunction(const PropNameID& name,
                                          ...
                                          HostFunctionType func) override {
    Around around{with_};
    return RD::createFunctionFromHostFunction(name, paramCount,
                                              std::move(func));
  };
  Value call(const Function& f, const Value& jsThis, const Value* args,
             size_t count) override {
 ...
 private:
  ...
  struct Around {
    ...
    ~Around() { detail::AfterCaller<With>::after(with_); }

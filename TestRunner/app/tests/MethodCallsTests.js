describe(module.id, function () {
    ...
    // Runs on both simulator and device: libffi 3.7.1 (this branch's libffi
    // update) fixed ObjC exception unwinding through ffi_call on the arm64
    // simulator, so the former isSimulator skip is obsolete.
    it('Throw_ObjC_exceptions_to_JavaScript', function () {
        try {
            ...
            expect(e.nativeException.reason).toContain("beyond bounds");
            expect(e.nativeException.name).toBe("NSRangeException");

            // The callstack-content assertions below are environmental (symbol
            // names differ across simulator/device); keep only a loose count check.
            expect(e.nativeException.callStackSymbols.count).toBeGreaterThan(0);
        }
    });

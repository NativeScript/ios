describe(module.id, function () {
    ...
    it("indexed assignment is a silent no-op for NSArray", function () {
        const arr = NSArray.arrayWithArray(['a', 'b', 'c']);
        // NSArray is exempt from the throw: the write is dropped without an error.
        expect(function () { arr[0] = 'ghost'; }).not.toThrow();
        expect(arr.count).toBe(3);
        expect(arr.objectAtIndex(0)).toBe('a');
    });
